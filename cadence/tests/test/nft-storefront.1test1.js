...
import { getMomentableCount, mintMomentable, getMomentable, typeID1 } from "../src/Momentables";
import {
	deployNFTStorefront,
	buyItem,
	sellItem,
...
describe("NFT Storefront", () => {
	...
	it("shall be able to create a sale offer", async () => {
		...
		await shallPass(mintMomentable(typeID1, Alice));

		const itemID = 0;

		await shallPass(sellItem(Alice, Number(itemID), toUFix64(1.11)));
	});
	...
	it("shall be able to accept a sale offer", async () => {
		...
		const Alice = await getAccountAddress("Alice");
		await setupStorefrontOnAccount(Alice);
		await mintMomentable(typeID1, Alice);
		...
		const itemCount = await getMomentableCount(Bob);
	...
	it("shall be able to remove a sale offer", async () => {
		...
		const item = await getMomentable(Alice, itemId);
